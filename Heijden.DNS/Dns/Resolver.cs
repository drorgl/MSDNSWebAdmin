...
namespace Heijden.DNS
{
	...
	public class Resolver
	{
		...
		private ConcurrentDictionary<string,Response> m_ResponseCache;
		...
		{
			m_ResponseCache = new ConcurrentDictionary<string, Response>();
		...
		private void AddToCache(Response response)
		{
            ...
            m_ResponseCache[strKey] = response;
		}
        ...
        /// <summary>
        /// Returns a list of KeyValuePair with name and IPAddress of Root DNS servers
        /// </summary>
        /// <param name="stream"></param>
        /// <returns></returns>
        public List<KeyValuePair<string, IPAddress>> LoadRootFile(Stream stream)
        {
            List<KeyValuePair<string, IPAddress>> records = new List<KeyValuePair<string, IPAddress>>();
            using (StreamReader sr = new StreamReader(stream))
            {
                while (!sr.EndOfStream)
                {
                    ...
                    string name = strLine.Substring(0, 24).Trim();
                    string ttl = strLine.Substring(24, 9).Trim();
                    string rclass = strLine.Substring(34, 4).Trim();
                    string rtype = strLine.Substring(38, 6).Trim();
                    string raddress = strLine.Substring(44, strLine.Length - 44).Trim();

                    IPAddress address = null;
                    if (IPAddress.TryParse(raddress, out address))
                        records.Add(new KeyValuePair<string,IPAddress>(name,address));
                    ...
                    //        break;
                    //    default:
                    //        throw new Exception(string.Format("Class {0} not implemented", rclass));
                    //        break;
                    //}
        ...
        /// <returns></returns>
        public List<KeyValuePair<string, IPAddress>> LoadRootFile(string strPath)
		{
            using (FileStream fs = new FileStream(strPath, FileMode.Open, FileAccess.Read))
            {
                return LoadRootFile(fs);
            }
        }


        /// <summary>
        /// Returns a list of KeyValuePair with name and IPAddress of Root DNS servers from Embedded Resource (named_root.txt)
        /// </summary>
        /// <returns></returns>
        public List<KeyValuePair<string, IPAddress>> LoadRootFile()
        {
            Stream rootfile = Assembly.GetExecutingAssembly().GetManifestResourceStream("Heijden.DNS.Dns.Root.named_root.txt");
            return LoadRootFile(rootfile);
        }
	} // class
}