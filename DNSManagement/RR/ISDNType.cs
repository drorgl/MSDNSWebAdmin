...
namespace DNSManagement.RR
{
    ...
    /// </summary>
    public class ISDNType : ResourceRecord
    {
        private ManagementObject m_mo;

        internal ISDNType(ManagementObject mo)
            : base(mo)
        {
            m_mo = mo;
        }
        ...
        public string ISDNNumber
        {
            get
            {
                return Convert.ToString(m_mo["ISDNNumber"]);
            }
        }
        ...
        public string SubAddress
        {
            get
            {
                return Convert.ToString(m_mo["SubAddress"]);
            }
        }
        ...
        /// <param name="isdnnumber">ISDN number and DDI of the record's owner.</param>
        /// <param name="subAddress">Subaddress of the owner, if defined.</param>
        /// <returns>the new object.</returns>
        public static ISDNType CreateInstanceFromPropertyData(
            Server server,
            ...
            TimeSpan? ttl,
            string isdnnumber, string subAddress)
        {
            if (server == null)
                throw new ArgumentNullException("server is required");

            ManagementClass dnsClass = new ManagementClass(server.m_scope, new ManagementPath("MicrosoftDNS_ISDNType"), null);
            ...
            //return new ISDNType((ManagementObject)dnsClass.InvokeMethod("CreateInstanceFromPropertyData", inParams, null));
            return new ISDNType(new ManagementObject(server.m_scope, new ManagementPath(dnsClass.InvokeMethod("CreateInstanceFromPropertyData", inParams, null)["RR"].ToString()), null));
        }
        ...
        /// </summary>
        /// <param name="ttl">Optional - Time, in seconds, that the RR can be cached by a DNS resolver.</param>
        /// <param name="isdnnumber">Optional - ISDN number and DDI of the record's owner.</param>
        /// <param name="subAddress">Optional - Subaddress of the owner, if defined.</param>
        /// <returns>the new object.</returns>
        public ISDNType Modify(TimeSpan? ttl, string isdnnumber, string subAddress)
        {
            ManagementBaseObject inParams = m_mo.GetMethodParameters("Modify");
            if ((ttl != null) && (ttl != this.TTL))
                inParams["TTL"] = ttl.Value.TotalSeconds;
            if ((!string.IsNullOrEmpty(isdnnumber)) && (isdnnumber != this.ISDNNumber))
                inParams["ISDNNumber"] = isdnnumber;
            if ((!string.IsNullOrEmpty(subAddress)) && (subAddress != this.SubAddress))
                inParams["SubAddress"] = subAddress;

            //return new ISDNType((ManagementObject)m_mo.InvokeMethod("Modify", inParams, null));
            return new ISDNType(new ManagementObject(m_mo.Scope, new ManagementPath(m_mo.InvokeMethod("Modify", inParams, null)["RR"].ToString()), null));
        }
        ...
        public override string ToString()
        {
            ...
            sb.AppendLineFormat("ISDNNumber={0}", ISDNNumber);
            sb.AppendLineFormat("SubAddress={0}", SubAddress);
